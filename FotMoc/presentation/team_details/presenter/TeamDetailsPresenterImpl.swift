...
class TeamDetailsPresenterImpl: TeamDetailsPresenter {
    ...
    func loadTeamData(teamId: String, leagueId: String) {
        ...
        Task { @MainActor in
            do {
                let team = try await teamDetailsUseCase.execute(teamId: teamId)
                let players = try await teamPlayersUseCase.execute(teamId: teamId)
                
                let stats = try? await teamStatsUseCase.execute(teamId: teamId, leagueId: leagueId)
                
                view?.displayTeamDetails(team: team, stats: stats, players: players)
            } catch {
