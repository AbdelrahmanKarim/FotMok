...
class TopScorersTab: LeagueTabManager {
    ...
    func numberOfItems(in section: Int) -> Int {
        ...
        func cell(for collectionView: UICollectionView, at indexPath: IndexPath) -> UICollectionViewCell {
            ...
            cell.configure(rank: scorer.rank, name: scorer.player.name, teamName: scorer.teamName, goals: scorer.goals, playerImageURL: scorer.player.imageUrl)
            return cell
        }
    ...
    func numberOfItemsInSectionSkeleton(section: Int) -> Int {
        return isLoading ? 5 : (topScorers.isEmpty ? 1 : topScorers.count)
    }

    
    func supplementaryView(for collectionView: UICollectionView, kind: String, at indexPath: IndexPath) -> UICollectionReusableView? { return nil }
    ...
    private func setupTopScorersSection(isEmpty: Bool) -> NSCollectionLayoutSection {
        ...
        let groupSize = NSCollectionLayoutSize(widthDimension: .fractionalWidth(1), heightDimension: isEmpty ? .absolute(350) : .absolute(120))
